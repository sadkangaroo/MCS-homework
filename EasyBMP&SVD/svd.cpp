 ...
 * 程序正常运行
 ********************************************************************/
int dluav(double a[],int m,int n,double u[],double v[],double eps)
{
    int ka;
    if (n > m) ka = n + 1; else ka = m + 1;
...
//矩阵相乘
void damul(double a[],double b[],int m,int n,int k,int m2, int n2, int k2, double c[])
{
	int i,j,l,u;
	for(i=0;i<=m-1;i++)
		for(j=0;j<=k-1;j++)
